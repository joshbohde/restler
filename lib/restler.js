...
var requestMethods = {
  ...
  _responseHandler: function(response) {
    ...
    if (this._isRedirect(response) && this.options.followRedirects == true) {
      ...
      response.addListener('end', function() {
        var emitBody = function(body){
            if (parseInt(response.statusCode) >= 400) self._respond('error',
                                                                    body,
                                                                    response);
            else self._respond('success', body, response);
            
            self._respond(response.statusCode.toString().replace(/\d{2}$/, 'XX'), 
                          body, 
                          response);
        ...
        if (self.options.parser){
            self.options.parser.call(response, body, emitBody);
        }else{
            emitBody(body);
        };
      });
  ...
  run: function() {
    ...
    if (this.options.multipart) {
      multipart.write(this.request, this.options.data, undefined, function() {
        self.request.end();
      });
    } else {
		this.request.end();
...
var parsers = {
    ...
    if (contentType) {
      if (contentType.indexOf('application/') == 0) {
        if (contentType.indexOf('json', 12) == 12)
          return parsers.json(data, callback);
        if (contentType.indexOf('xml', 12) == 12)
          return parsers.xml(data, callback);
        if (contentType.indexOf('yaml', 12) == 12)
          return parsers.yaml(data, callback);
      }
    }
  ...
  xml: function(data, callback) {
    parser = new x2j.Parser();
    parser.addListener('end', callback);
    return data && parser.parseString(data);
  },
